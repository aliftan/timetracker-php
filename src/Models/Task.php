...
class Task {
    ...
    public function create($data) {
        $stmt = $this->db->prepare('
            INSERT INTO tasks (project_id, name, description, estimated_hours, status) 
            VALUES (:project_id, :name, :description, :estimated_hours, :status)
        ');
        
        $stmt->bindValue(':project_id', $data['project_id'], SQLITE3_INTEGER);
        $stmt->bindValue(':name', $data['name'], SQLITE3_TEXT);
        $stmt->bindValue(':description', $data['description'], SQLITE3_TEXT);
        $stmt->bindValue(':estimated_hours', $data['estimated_hours'], SQLITE3_FLOAT);
